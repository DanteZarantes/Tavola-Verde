    ...
    <header className="bg-gradient-to-r from-primary via-primary/98 to-primary text-white shadow-2xl sticky top-0 z-50 backdrop-blur-sm">
      <div className="max-w-7xl mx-auto px-4 py-4">
        <div className="flex items-center justify-between">
          <Link href="/" className="text-3xl font-bold text-gold hover:text-gold/80 transition-all transform hover:scale-105 font-serif">
            Tavola Verde
          </Link>
          ...
          <nav className="hidden md:flex items-center gap-8">
            ...
            <Link href="/contacts" className="text-cream hover:text-gold transition-all font-medium text-lg relative group">
              Контакты
              <span className="absolute bottom-0 left-0 w-0 h-0.5 bg-gold group-hover:w-full transition-all"></span>
            </Link>
            {user && (
              <Link href="/orders" className="text-cream hover:text-gold transition-all font-medium text-lg relative group">
                Заказы
                <span className="absolute bottom-0 left-0 w-0 h-0.5 bg-gold group-hover:w-full transition-all"></span>
              </Link>
            )}
            ...
            <Link href="/cart" className="relative group">
              <div className="flex items-center gap-2 bg-gold text-primary px-5 py-2.5 rounded-full font-semibold hover:bg-gold/90 transition-all transform hover:scale-105 shadow-lg">
                ...
                <span>Корзина</span>
                {itemCount > 0 && (
                  <span className="bg-primary text-gold rounded-full w-6 h-6 flex items-center justify-center text-sm font-bold animate-pulse">
                    {itemCount}
                  </span>
                )}
              </div>
        ...
        {mobileMenuOpen && (
          <nav className="md:hidden mt-6 flex flex-col gap-4 pb-4 animate-slideIn">
            ...
            <Link href="/contacts" className="text-cream hover:text-gold transition-all font-medium text-lg py-2" onClick={() => setMobileMenuOpen(false)}>
              Контакты
            </Link>
            {user && (
              <Link href="/orders" className="text-cream hover:text-gold transition-all font-medium text-lg py-2" onClick={() => setMobileMenuOpen(false)}>
                Заказы
              </Link>
            )}
